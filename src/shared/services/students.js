...
/**
 * Client-side filter over an already-fetched student list by department,
 * level, or a free-text match against name/index_number. This filters
 * in-memory rather than adding more service calls, since listStudents()
 * already returns the full RLS-scoped set for the caller's role — for an
 * admin that's every student, which is the dataset Section 9.7 / FR7's
 * admin pages are expected to search and filter client-side.
 */
export function filterStudents(students, { department, level, search } = {}) {
  let rows = students;
  if (department) {
